...
class Glove(Node):
    def __init__(self, node_name='glove'):
        ...
        self.df = pd.read_csv('/home/feld/ros2_ws/datasets/corrupted_dataset_filtered_usable/bottle/data_1750948857014344353.txt', header=None)
    ...
    def publish_glove_data(self):
        ...
        if self.row_sent < self.df.shape[0]:
            ...
            msg.quat_hand = np.array(row[25:29], dtype=np.float32)
            msg.quat_forearm = np.array(row[29:33], dtype=np.float32)
